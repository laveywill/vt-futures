...
```{r}
create_county_plot_3 <- function(df, county) {
  ...
  county_data_long <- county_data %>%
    ...
    mutate(Population_Type = factor(Population_Type, 
           ...
           `Age Groups` = gsub("_", " ", `Age Groups`))  # Replace underscores with spaces
  
  # Compute the maximum stacked population across age groups
  ...
  # Optionally add a small margin (e.g., 10%) above the maximum value
  upper_limit <- max_population * 1.1
  
  p <- ggplot(county_data_long, aes(x = reorder(`Age Groups`, Population), y = Population, fill = Population_Type)) +
    geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
    coord_flip() +
...
create_county_plot_3(cleaned_df_age_distrbution, "bennington")
...
create_county_plot_4 <- function(df, county) {
  ...
  p <- ggplot(county_data_long, aes(x = reorder(`Age Groups`, total_population), 
...
create_county_plot_4(cleaned_df_age_distrbution, "caledonia")
```
